...
export default function News() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            console.log("SIUUJU", featuredArticleResponse )


            // 3. Use the `set` functions defined above to update the `articles` and `featuredArticle` variables
            const featured_article = await featuredArticleResponse.json()
            console.log("the Feautred article", featured_article)
            const articles = await articlesResponse.json()
            console.log("articles", articles)

        
            setFeaturedArticle(featured_article);
